...
import { MdDashboard, MdInventory, MdShoppingBag } from 'react-icons/md';
import { FiUser } from 'react-icons/fi';

const links = [
  { to: '/vendor/dashboard', icon: <MdDashboard size={18} />, label: 'Dashboard' },
  { to: '/vendor/products', icon: <MdInventory size={18} />, label: 'My Products' },
  { to: '/vendor/orders', icon: <MdShoppingBag size={18} />, label: 'Orders' },
...
const VendorSidebar = () => (
  <aside className="w-56 bg-white border-r border-gray-200 min-h-screen flex-shrink-0">
    <Link to="/" className="flex items-center gap-2 px-5 py-4 border-b border-gray-200 bg-[#2874f0]">
      ...
      <span className="text-[#ffe500] text-xs font-medium">Seller</span>
    </Link>
    <nav className="py-2">
